...
literals math math.parser sequences sets splitting ;
...
: clean-haiku ( haiku extra -- words )
    [ >lower " .\n" split harvest ]
    [ '[ _ member? ] reject " " join ] bi* ;

: decode-key ( haiku key -- seq )
    [ [ ?head ] find drop [ [ CHAR: \s = ] trim-head ] dip ] map nip ;

: decode-haiku ( haiku extra key -- seq )
    [ clean-haiku ] [ decode-key ] bi* ;

PRIVATE>

: ipv4>haiku ( ipv4 -- haiku )
    parse-ipv4 split-octets
    ipv4-key encode-key
    ipv4-schema encode-haiku ;

: haiku>ipv4 ( haiku -- ipv4 )
    { "in" "the" } ipv4-key decode-haiku join-octets be> ipv4-ntoa ;

: ipv6>haiku ( ipv6 -- haiku )
    parse-ipv6 split-bytes
    ipv6-key encode-key
    ipv6-schema encode-haiku ;

: haiku>ipv6 ( haiku -- ipv6 )
    { "and" } ipv6-key decode-haiku be> ipv6-ntoa ;

: haiku>ip ( haiku -- ipv4/ipv6 )
    " and " over subseq? [ haiku>ipv6 ] [ haiku>ipv4 ] if ;

: ip>haiku ( ipv4/ipv6 -- haiku )
    CHAR: : over index [ ipv6>haiku ] [ ipv4>haiku ] if ;
